...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        if (!Session::has('cart')) {
            ...
            $selectable = ['id','user_id','name','sku','slug','features','colors','thumbnail','price','previous_price','attributes','size','size_price','discount_date'];
            $ps = DB::table('pagesettings')->find(1); 
            $feature_products =  Product::with('user')->where('category_id','=',26)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(8)->get()->reject(function($item){
            ...
            return view('front.cart', compact('ps','feature_products'));
        }
        ...
        return view('front.cart', compact('ps','products','totalPrice','mainTotal','feature_products','tx')); 
    }
   ...
   public function addtocart($id)
    {
        $prod = Product::where('id','=',$id)->first(['id','user_id','sku','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes']);
        ...
        //$checkCart = new Cart($oldCart);
        
        //$checkCart->add($prod, $prod->id, $size ,$color, $keys, $values);
        
        /*
        return redirect()->route('front.cart')->with('unsuccess',$oldCart);
          
        if((!empty($checkCart)) && ($checkCart->count == 0) && ($checkCart[0]['category_id'] == 26 ) ) {
            return redirect()->route('front.cart')->with('unsuccess',$lang->lang77884455);
        }
       */
        
        /*
        $checkCart->add($prod, $prod->id, $size ,$color, $keys, $values);
        if($checkCart->items[$id.$size.$color.str_replace(str_split(' ,'),'',$values)]['category_id'] == 26) {
            return redirect()->route('front.cart')->with('unsuccess',$lang->out_stock);
        }
        */
